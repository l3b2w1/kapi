...
{
	int result=-1;
  	printk("<1>tasklet running. by author\n");
	printk("<1>in irq_tasklet_action the state of the tasklet is :%ld\n",(&tasklet)->state);  //显示当前中断的状态
	result=tasklet_trylock(&tasklet);                            //给中断加锁
	if(result==0)                                                //显示加锁结果
		printk("<1>tasklet_trylock failed\n");
	else
		printk("<1>tasklet_trylock success\n");
	printk("<1>out irq_tasklet_action\n");
   	return;
}
