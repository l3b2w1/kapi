...
{
	printk("<1> In the my_timer_function\n");
	
	printk("<1>the jiffies is :%ld\n",jiffies);//显示当前节拍数    
	struct timer_list *mytimer = (struct timer_list *)data;	
	
	printk("<1>the expries of my_timer1 is :%ld\n",mytimer->expires);//显示定时器到期节拍数
}
...
int __init timer_init(void)
{
    ...
    /* this timer is deleted before invoking */
    ...
    /* this timer works */
	my_timer.expires = jiffies + 20*HZ;  // HZ=250;初始化字段expires	
